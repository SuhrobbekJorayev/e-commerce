...
class ProductViewSetTestCase(APITestCase):
    def setUp(self):
        ...
        self.product1 = Product.objects.create(name='Laptop', description='Powerful Laptop', category=self.category1,
                                               price=5000)
        self.product2 = Product.objects.create(name='Book', description='Thriller Book', category=self.category2,
                                               price=6000)

        Review.objects.create(product=self.product1, rating=5, user_id=1)
        Review.objects.create(product=self.product1, rating=4, user_id=3)
    ...
    def test_product_detail(self):
        ...
        self.client.force_authenticate(self.user)
        response = reverse(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data['product']['name']), 'laptop')
